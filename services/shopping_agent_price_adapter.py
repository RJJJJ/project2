...
def build_agent_price_plan(
    ...
    planner_result = plan_cheapest_by_product_candidates(db_path, point_code, priceable_items, strategy=strategy)
    ...
    best_plan = planner_result.get("best_plan")
    status = _adapter_status(
        str(planner_result.get("status")),
    ...
    return {
        ...
        "ambiguous_items": ambiguous_items,
        "not_covered_items": not_covered_items,
        "store_plans": planner_result.get("store_plans") or [],
        "best_plan": best_plan,
        "warnings": list(dict.fromkeys(warnings)),
        "diagnostics": {
            **(planner_result.get("diagnostics") or {}),
            "priceable_item_count": len(priceable_items),
            "unpriceable_item_count": len(unpriceable_items),
            "ambiguous_count": len(ambiguous_items),
            "not_covered_count": len(not_covered_items),
            "plan_count": len(planner_result.get("store_plans") or []),
        },
    }
